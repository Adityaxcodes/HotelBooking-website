...
const HotelCard = ({room, index}) => {
  return (
    <Link to={'/rooms/' + room._id} onClick={() => scrollTo(0, 0)} key={room._id} className="block bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300 relative max-w-xs">
      {/* Best Seller Tag */}
      {index % 2 !== 0 && (
      ...
      <div className="p-3">
        ...
        <div className="flex items-center mt-2 text-gray-600">
          ...
          <span className="text-sm ml-1">{room.hotel.address}</span>
        </div>
      </div>
